(function () {
    ...
    function AppleTVComponent(object) {
        ...
        this.create = function () {
            ...
            // Стили обычной строкой (ES5), чтобы старый WebKit не ругался
            ...
            style.textContent = 
                ...
                ".atv-carousel { display: flex; transition: transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1); }" +
                ".atv-card { width: 220px; height: 330px; background: #333; border-radius: 12px; margin-right: 20px; transition: transform 0.2s ease, border 0.2s ease; border: 4px solid transparent; flex-shrink: 0; background-size: cover; background-position: center; }" +
            ...
            var carouselHTML = '';
            var posterUrl = 'https://image.tmdb.org/t/p/w300/qJ2tW6WMUDux911r6m7haRef0WH.jpg';
            
            for (var i = 0; i < 10; i++) {
                carouselHTML += '<div class="atv-card" style="background-image: url(\\'' + posterUrl + '\\')"></div>';
            }

            content.innerHTML = '<div class="atv-title">В тренде</div><div class="atv-carousel">' + carouselHTML + '</div>';
    ...
    function initAppleTVPlugin() {
        window.plugin_appletv_ready = true;
        Lampa.Component.add('appletv_page', AppleTVComponent);

        var item = $('<li class="menu__item selector"><div class="menu__ico"><svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M21 3H3C1.89 3 1 3.89 1 5v14c0 1.11.89 2 2 2h18c1.11 0 2-.89 2-2V5c0-1.11-.89-2-2-2zm0 16H3V5h18v14z"/></svg></div><div class="menu__text">Apple TV+</div></li>');
    ...
    if (!window.plugin_appletv_ready) {
